    ...
    <main class="flex flex-col justify-center items-center">
        <div id="cursor" class="opacity-0 m-2 fixed transition-transform duration-500 pointer-events-none z-9999">
        ...
        <section class="relative w-full max-w-screen-3xl h-lvh min-h-[800px] p-2.5 transition-opacity duration-500">
            <div id="main"
                class="relative overflow-hidden flex flex-col items-start justify-start w-full h-full min-h-[780px] max-w-screen-3xl p-2 bg-dark"
                style="clip-path: inset(0% 100% 0% 0%);">
                <div class="w-full h-full absolute top-0 left-0 z-40 pointer-events-none">
                    {{ $slot }}
                </div>
                <div class="relative z-30 flex flex-col-reverse md:flex-row items-start justify-between w-full gap-2">
                    ...
                    @endif --}}
                        @if (request()->routeIs('home'))
                            <button id="resetBtn" class="button hidden lg:block glitch">{Clear Cache}</button>
                        @endif
                    </div>
                </div>
                <div
